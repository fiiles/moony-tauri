...
use crate::models::{Loan, InsertLoan};
use tauri::State;
use uuid::Uuid;
    ...
    db.with_conn(|conn| {
        conn.execute(
            "UPDATE loans SET name = ?1, principal = ?2, 
             ...
             WHERE id = ?10",
            rusqlite::params![
                data.name, data.principal, data.currency, data.interest_rate,
                data.interest_rate_validity_date, data.monthly_payment, data.start_date,
                data.end_date, now, id
            ],
        )?;
