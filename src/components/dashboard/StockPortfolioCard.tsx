...
export default function StockPortfolioCard({ holdings, onAdd }: StockPortfolioCardProps) {
  ...
  const totalValue = holdings.reduce(
    (sum, stock) => sum + calculateMarketValue(stock.shares, stock.currentPrice),
    0
  );
      ...
      <div className="space-y-3">
          ...
          <div className="space-y-1">
            ...
            <div
              className={`flex items-center gap-1 text-xs ${
                totalGainLoss >= 0 ? 'text-chart-2' : 'text-destructive'
              }`}
            >
              {totalGainLoss >= 0 ? (
              ...
              <span className="tabular-nums">
                {totalGainLoss >= 0 ? '+' : ''}
                {formatCurrency(Math.abs(totalGainLoss))}({totalGainLossPercent >= 0 ? '+' : ''}
                {totalGainLossPercent.toFixed(2)}%)
              </span>
            </div>
          </div>
          <span className="text-2xl font-bold tabular-nums">{formatCurrency(totalValue)}</span>
        </div>
        ...
        {holdings.map((stock) => {
          ...
          return (
            <div
              key={stock.id}
              className="flex justify-between items-center py-2 hover-elevate rounded-md px-2 -mx-2"
            >
              <div>
              ...
              <div className="text-right">
                <p className="font-semibold tabular-nums text-sm">{formatCurrency(currentValue)}</p>
                <p
                  className={`text-xs tabular-nums ${
                    gainLoss >= 0 ? 'text-chart-2' : 'text-destructive'
                  }`}
                >
                  {gainLoss >= 0 ? '+' : ''}
