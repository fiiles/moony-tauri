...
export function CsvImportDialog({
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const result = await bankAccountsApi.importCsvTransactions(accountId, filePath, config);
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {step === 'select' &&
              t('csvImport.selectFile', 'Select a CSV file exported from your bank')}
            {step === 'preview' &&
              t('csvImport.mapColumns', 'Map CSV columns to transaction fields')}
          ...
          <div className="py-8">
            ...
            >
              ...
              <p className="text-lg font-medium">
                {t('csvImport.clickToSelect', 'Click to select CSV file')}
              </p>
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-2 p-3 bg-muted rounded-lg">
              ...
              <span className="text-muted-foreground">
                ({preview.totalRows} {t('csvImport.rows', 'rows')})
              </span>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="grid gap-2">
                  <Label>
                    {t('csvImport.dateColumn', 'Date Column')} *{getConfidenceBadge('date')}
                  </Label>
                ...
                <div className="grid gap-2">
                  <Label>
                    {t('csvImport.amountColumn', 'Amount Column')} *{getConfidenceBadge('amount')}
                  </Label>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="grid gap-2">
                    ...
                    <TooltipProvider>
                      <Tooltip>
                        ...
                        <TooltipContent side="top" className="max-w-[250px]">
                          <p>
                            {t(
                              'csvImport.descriptionColumnsHelp',
                              'Select all available descriptive fields (e.g., message, note, purpose) for better automatic categorization.'
                            )}
                          </p>
                        </TooltipContent>
                      </Tooltip>
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                        variant="outline"
                        className="w-full justify-between font-normal h-10 text-left overflow-hidden"
                      >
                      ...
                      >
                        {preview.headers
                          ...
                          .map((h) => (
                            <label
                              key={h}
                              className="flex items-center gap-3 text-sm cursor-pointer hover:bg-muted p-2 rounded"
                            >
                              <Checkbox
                                ...
                                onCheckedChange={(checked) => {
                                  if (checked) {
                                    ...
                                    setDescriptionColumns(
                                      descriptionColumns.filter((c) => c !== h)
                                    );
                                  }
                                }}
                              />
                ...
                <div className="grid gap-2">
                  ...
                  <Select
                    value={counterpartyColumn || '__none__'}
                    onValueChange={(v) => setCounterpartyColumn(v === '__none__' ? '' : v)}
                  >
                    <SelectTrigger>
                ...
                <div className="grid gap-2">
                  ...
                  <Select
                    value={variableSymbolColumn || '__none__'}
                    onValueChange={(v) => setVariableSymbolColumn(v === '__none__' ? '' : v)}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder={t('csvImport.optional', 'Optional')} />
                    </SelectTrigger>
                ...
                <div className="grid gap-2">
                  ...
                  <Select
                    value={currencyColumn || '__none__'}
                    onValueChange={(v) => setCurrencyColumn(v === '__none__' ? '' : v)}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder={t('csvImport.optional', 'Optional')} />
                    </SelectTrigger>
                ...
                <div className="grid gap-2">
                  ...
                  <Select
                    value={counterpartyIbanColumn || '__none__'}
                    onValueChange={(v) => setCounterpartyIbanColumn(v === '__none__' ? '' : v)}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder={t('csvImport.optional', 'Optional')} />
                    </SelectTrigger>
            ...
            <div className="border rounded-lg">
              ...
              >
                ...
                <span className="text-muted-foreground text-sm">
                  {preview.headers.length} {t('csvImport.columns', 'columns')}, {preview.totalRows}{' '}
                  {t('csvImport.rows', 'rows')}
                </span>
              </Button>
              {showPreview && (
