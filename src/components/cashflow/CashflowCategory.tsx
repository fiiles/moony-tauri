      ...
      <Collapsible open={isOpen} onOpenChange={setIsOpen}>
        <CardHeader className="p-4">
          <div className="flex items-center justify-between">
            ...
            <span className="font-semibold">{formatAmount(category.total)}</span>
          </div>
        </CardHeader>
        ...
        <CollapsibleContent>
          <CardContent className="pt-0 pb-4 px-4">
            {hasItems ? (
                    ...
                    <div className="flex flex-col">
                      ...
                      {item.isUserDefined && (
                        <span className="text-xs text-muted-foreground">
                          {item.originalAmount.toLocaleString()} {item.originalCurrency} /{' '}
                          {item.originalFrequency === 'monthly' ? t('perMonth') : t('perYear')}
                        </span>
                      )}
                    </div>
              ...
              <p className="text-sm text-muted-foreground text-center py-4">{t('empty.noItems')}</p>
            )}
          </CardContent>
        </CollapsibleContent>
      </Collapsible>
