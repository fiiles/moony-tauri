  ...
  const sortedLoans = useMemo(() => {
    return [...loans].sort((a, b) => {
      ...
      switch (sortColumn) {
        ...
        case 'rateValidity':
          comparison =
            Number(a.interestRateValidityDate || 0) - Number(b.interestRateValidityDate || 0);
          break;
        case 'endDate':
          ...
          <Table>
            <TableHeader className="[&_th]:bg-muted/50">
              <TableRow>
                ...
                <TableHead
                  className="text-right cursor-pointer select-none hover:bg-muted/50"
                  onClick={() => handleSort('rateValidity')}
                >
                  <span className="flex items-center justify-end">
                    {t('table.rateValidity')}
                    <SortIcon column="rateValidity" />
                  </span>
                </TableHead>
                <TableHead
                  className="text-right cursor-pointer select-none hover:bg-muted/50"
            ...
            <TableBody>
              {sortedLoans.length === 0 ? (
                ...
                sortedLoans.map((loan) => {
                  const isMatured = loan.endDate
                    ? isAfter(today, new Date(Number(loan.endDate) * 1000))
                    : false;
                  return (
                    <TableRow key={loan.id} className={isMatured ? 'opacity-50 bg-muted/50' : ''}>
                      <TableCell className="font-medium">{loan.name}</TableCell>
                      <TableCell className="text-right">
                        {formatCurrency(
                          convertToCzK(Number(loan.principal), loan.currency as CurrencyCode)
                        )}
                      </TableCell>
                      <TableCell className="text-right">
                      ...
                      <TableCell className="text-right">
                        {loan.endDate ? formatDate(new Date(Number(loan.endDate) * 1000)) : '—'}
                      </TableCell>
                      <TableCell className="text-right">
                        <DropdownMenu>
