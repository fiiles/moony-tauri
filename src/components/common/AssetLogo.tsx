...
export function AssetLogo({ ticker, type, className }: AssetLogoProps) {
  ...
  const fallbackBg = type === 'stock' ? getInstrumentIcon(ticker) : 'bg-orange-100 text-orange-600';

  const initials = type === 'stock' ? ticker.substring(0, 2) : ticker.substring(0, 3);

  return (
    <Avatar className={cn('h-10 w-10 bg-white', className)}>
      <AvatarImage src={logoUrl} alt={ticker} className="object-contain p-1" />
      <AvatarFallback
        className={cn('font-bold text-sm', fallbackBg, type === 'stock' && 'text-white')}
      >
        {initials}
      </AvatarFallback>
    </Avatar>
