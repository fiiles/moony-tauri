...
export function ViewTransactionsModal({
  ...
  const [selectedTransaction, setSelectedTransaction] = useState<InvestmentTransaction | null>(
    null
  );
  ...
  const { data: transactions, isLoading } = useQuery<InvestmentTransaction[]>({
    ...
    queryFn: () =>
      investment ? investmentsApi.getTransactions(investment.id) : Promise.resolve([]),
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {t('modals.transactions.title')} - {investment.ticker}
            </DialogTitle>
            <DialogDescription>{t('actions.viewTransactions')}</DialogDescription>
          </DialogHeader>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              {t('modals.transactions.noTransactions')}
            </div>
          ) : (
                ...
                <TableBody>
                  {transactions.map((tx) => (
                    <TableRow key={tx.id} className="row-interactive">
                      <TableCell>
                        {new Date(tx.transactionDate * 1000).toLocaleDateString()}
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell className="text-right">
                        {parseFloat(tx.quantity as any).toFixed(4)}
                      </TableCell>
                      <TableCell className="text-right">
                        {formatAmount(parseFloat(tx.pricePerUnit as any), tx.currency)}{' '}
                        {tx.currency}
                      </TableCell>
                      <TableCell>{tx.currency}</TableCell>
                      <TableCell className="text-right">
                        {formatAmount(
                          parseFloat(tx.pricePerUnit as any) * parseFloat(tx.quantity as any),
                          tx.currency
                        )}{' '}
                        {tx.currency}
                      </TableCell>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>{tc('confirm.deleteDescription')}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
