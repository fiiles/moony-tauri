...
export function InvestmentsSummary({
  ...
  const realizedTitle = realizedGain >= 0 ? t('summary.realizedGain') : t('summary.realizedLoss');

  const dividendCard = (
    <SummaryCard
      title={t('summary.estimatedDividend')}
      value={formatCurrency(metrics.estimatedDividendYield, {
  ...
  return (
    ...
    >
      <SummaryCard
        ...
        subtitle={
          latestFetchedAt
            ? t('summary.lastUpdated', { date: latestFetchedAt.toLocaleDateString() })
            : undefined
        }
      />
      ...
      <SummaryCard
        ...
        value={formatCurrencyRaw(Math.abs(metrics.overallGainLoss), {
          minimumFractionDigits: 0,
          maximumFractionDigits: 0,
        })}
        icon={
          metrics.overallGainLoss >= 0 ? (
      ...
      {hasRealizedGain ? (
        <>
          <SummaryCard
            title={realizedTitle}
            value={formatCurrencyRaw(Math.abs(realizedGain), {
              minimumFractionDigits: 0,
              maximumFractionDigits: 0,
            })}
            icon={
          ...
          <SummaryCard
            ...
            subtitle={
              metrics.topPerformer
                ? `+${metrics.topPerformer.gainLossPercent.toFixed(2)}%`
                : undefined
            }
            valueClassName="text-positive"
          />
