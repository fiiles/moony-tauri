...
import { ArrowLeft, Building2, DollarSign, Home, Trash2, Pencil, Wallet } from 'lucide-react';
...
export default function RealEstateDetail() {
  ...
  const totalLoansInCzk =
    linkedLoans?.reduce((sum, loan) => {
  ...
  const marketCurrency =
    (realEstate as unknown as { marketPriceCurrency?: string }).marketPriceCurrency || 'CZK';
  ...
  const totalRecurringCostsInCzk =
    (
      realEstate.recurringCosts as unknown as {
  ...
  const rentCurrency =
    (realEstate as unknown as { monthlyRentCurrency?: string }).monthlyRentCurrency || 'CZK';
  const rentInCzk = convertToCzK(monthlyRent * 12, rentCurrency as CurrencyCode);

  const yearlyLoanPaymentsInCzk =
    linkedLoans?.reduce((sum, loan) => {
  ...
  const yearlyInsurancePaymentsInCzk =
    linkedInsurances?.reduce((sum, ins) => {
  ...
  const netCashflowInCzk =
    rentInCzk - totalRecurringCostsInCzk - yearlyLoanPaymentsInCzk - yearlyInsurancePaymentsInCzk;
    ...
    <div className="p-6 md:p-8 lg:p-10 max-w-7xl mx-auto">
      ...
      <div className="grid gap-4 md:grid-cols-4 mb-8">
        <Card className="card-hover">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              ...
              {formatCurrency(
                convertToCzK(
                  Number(realEstate.purchasePrice),
                  ((realEstate as unknown as { purchasePriceCurrency?: string })
                    .purchasePriceCurrency as CurrencyCode) || 'CZK'
                )
              )}
            </p>
          </CardContent>
        ...
        <Card className="card-hover">
          ...
          <CardContent>
            <div
              className={`text-2xl font-bold ${netCashflowInCzk >= 0 ? 'text-green-600' : 'text-red-600'}`}
            >
                        ...
                        <TableCell className="text-right">
                          {formatCurrency(
                            convertToCzK(
                              ...
                              ((loan as unknown as { currency?: string })
                                .currency as CurrencyCode) || 'CZK'
                            )
                          )}
                        </TableCell>
                  ...
                  {(
                    ...
                    <TableRow key={i}>
                      ...
                      <TableCell className="capitalize">
                        {t('modal.add.' + cost.frequency)}
                      </TableCell>
                      <TableCell className="text-right">
                        {formatCurrency(
                          convertToCzK(
                            Number(cost.amount),
                            (cost.currency as CurrencyCode) || 'CZK'
                          )
                        )}
                      </TableCell>
                    </TableRow>
                  ))}
                  {(!realEstate.recurringCosts ||
                    (realEstate.recurringCosts as unknown as unknown[]).length === 0) && (
                    <TableRow>
                    ...
                    <TableRow key={cost.id}>
                      ...
                      <TableCell className="text-right">
                        {formatCurrency(
                          convertToCzK(
                            Number(cost.amount),
                            ((cost as unknown as { currency?: string }).currency as CurrencyCode) ||
                              'CZK'
                          )
                        )}
                      </TableCell>
