...
export default function BankAccounts() {
  ...
  const EffectiveRateCell = ({ item }: { item: BankAccountWithInstitution }) => {
    ...
    const { effectiveRate } = useZonedEffectiveRate(
      item.id,
      balance,
      item.hasZoneDesignation || false
    );
                  ...
                  {sortedAccounts.map((item) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell>
                          <Badge variant="secondary">{t(`accountTypes.${item.accountType}`)}</Badge>
                        </TableCell>
                        <TableCell className="text-right">
                          {formatCurrency(
                            convertToCzK(
                              parseFloat(item.balance),
                              (item.currency || 'CZK') as CurrencyCode
                            )
                          )}
                        </TableCell>
