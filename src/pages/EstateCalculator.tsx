  ...
  const projection = useMemo(() => {
    ...
    if (amortizationSchedule.length > 0 && monthsInDuration > 0) {
      const scheduleIndex = Math.min(
        Math.floor(monthsInDuration) - 1,
        amortizationSchedule.length - 1
      );
      remainingLoanBalance =
        scheduleIndex >= 0 ? amortizationSchedule[scheduleIndex].remainingBalance : 0;
    }
    ...
    const finalValueMultiple =
      ownCapital > 0 ? (netEquityAtEnd + cumulativeNetCashflow) / ownCapital : 0;
    ...
    const annualizedYield =
      ownCapital > 0 && duration > 0 && finalValueMultiple > 0
        ...
        <CardContent className="pt-6">
          ...
          <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="p-4 bg-muted/50 rounded-lg border">
              <div className="flex justify-between items-center">
                ...
                <span className="text-lg font-semibold text-primary">
                  {formatCurrencyRaw(calculations.ownCapital)}
                </span>
              </div>
              <div className="flex justify-between items-center mt-1">
                <span className="text-xs text-muted-foreground">
                  {t('estate.form.ownCapitalDescription')}
                </span>
                <span className="text-xs text-muted-foreground">
                  {t('estate.form.ltv')}: {calculations.ltv.toFixed(1)}%
                </span>
              </div>
            </div>
            ...
            <div className="p-4 bg-muted/50 rounded-lg border">
              ...
              <div className="flex justify-between items-center mt-1">
                <span className="text-xs text-muted-foreground">
                  {t('estate.form.monthlyLoanPaymentDescription')}
                </span>
        ...
        <CardContent className="pt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                {t('estate.form.monthlyExpensesDescription')}
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                {t('estate.form.unoccupiedMonthsDescription')}
              </p>
            </div>
          </div>
          ...
          <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="p-4 bg-muted/50 rounded-lg border">
              <div className="flex justify-between items-center">
                ...
                <span
                  className={`text-lg font-semibold ${calculations.grossCashflow >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
                >
              ...
              <div className="flex justify-between items-center mt-1">
                <span className="text-xs text-muted-foreground">
                  {t('estate.form.grossCashflowDescription')}
                </span>
                <span className="text-xs text-muted-foreground">{t('estate.form.perMonth')}</span>
              </div>
            </div>
            ...
            <div className="p-4 bg-muted/50 rounded-lg border">
              <div className="flex justify-between items-center">
                ...
                <span
                  className={`text-lg font-semibold ${calculations.netCashflow >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
                >
              ...
              <div className="flex justify-between items-center mt-1">
                <span className="text-xs text-muted-foreground">
                  {t('estate.form.netCashflowDescription')}
                </span>
                <span className="text-xs text-muted-foreground">{t('estate.form.perMonth')}</span>
              </div>
            </div>
          ...
          <div className="mt-4 p-4 bg-primary/5 border border-primary/20 rounded-lg">
            <div className="flex justify-between items-center">
              ...
              <span
                className={`text-2xl font-bold ${calculations.yieldOnCapital >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
              >
            ...
            <div className="flex justify-between items-center mt-1">
              <span className="text-xs text-muted-foreground">
                {t('estate.form.yieldDescription')}
              </span>
        ...
        <CardContent className="pt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-6">
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-muted-foreground">
                {t('estate.projection.oneTimeCostsDescription')}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div className="p-4 bg-muted/50 rounded-lg border">
              ...
              <div className="flex justify-between items-center mt-1">
                <span className="text-xs text-muted-foreground">
                  {t('estate.projection.afterYears', { years: investmentDuration })}
                </span>
                <span className="text-xs text-muted-foreground">
                  +
                  {formatCurrencyRaw(
                    projection.finalEstatePrice - (parseFloat(purchasePrice) || 0)
                  )}
                </span>
              </div>
            </div>
            ...
            <div className="p-4 bg-muted/50 rounded-lg border">
              ...
              <div className="flex justify-between items-center mt-1">
                <span className="text-xs text-muted-foreground">
                  {t('estate.projection.paidOff')}
                </span>
                <span className="text-xs text-muted-foreground">
                  {formatCurrencyRaw(
                    (parseFloat(loanAmount) || 0) - projection.remainingLoanBalance
                  )}
                </span>
              </div>
            </div>
            ...
            <div className="col-span-1 md:col-span-2 p-4 bg-muted/50 rounded-lg border">
              <div className="flex justify-between items-center">
                <span className="text-sm font-medium">
                  {t('estate.projection.earningsFromSale')}
                </span>
                <span
                  className={`text-lg font-semibold ${projection.finalEstatePrice - projection.remainingLoanBalance - calculations.ownCapital >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
                >
                  {formatCurrencyRaw(
                    projection.finalEstatePrice -
                      projection.remainingLoanBalance -
                      calculations.ownCapital
                  )}
                </span>
              </div>
              <div className="flex justify-between items-center mt-1">
                <span className="text-xs text-muted-foreground">
                  {t('estate.projection.earningsFromSaleDescription')}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div className="p-4 bg-muted/50 rounded-lg border">
              <div className="flex justify-between items-center">
                <span className="text-sm font-medium">
                  {t('estate.projection.positiveCashflow')}
                </span>
                <span
                  className={`text-lg font-semibold ${projection.positiveMonthCashflow ? 'text-green-600 dark:text-green-400' : 'text-muted-foreground'}`}
                >
              ...
              <div className="flex justify-between items-center mt-1">
                <span className="text-xs text-muted-foreground">
                  {t('estate.projection.positiveCashflowDescription')}
                </span>
                {projection.positiveMonthCashflow && (
                  <span className="text-xs text-muted-foreground">
            ...
            <div className="p-4 bg-muted/50 rounded-lg border">
              <div className="flex justify-between items-center">
                <span className="text-sm font-medium">
                  {t('estate.projection.cumulativeRentalEarnings')}
                </span>
                <span
                  className={`text-lg font-semibold ${projection.cumulativeNetCashflow >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
                >
              ...
              <div className="flex justify-between items-center mt-1">
                <span className="text-xs text-muted-foreground">
                  {t('estate.projection.overDuration')}
                </span>
                <span className="text-xs text-muted-foreground">
                  {investmentDuration} {t('estate.form.years')}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 bg-primary/5 border border-primary/20 rounded-lg">
            <div className="flex justify-between items-center">
              ...
              <div className="text-right">
                <span
                  className={`text-2xl font-bold ${projection.projectionYield >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
                >
                  {formatPercentage(projection.projectionYield)}
                </span>
                <span className="text-sm text-muted-foreground ml-2">
                  ({formatPercentage(projection.annualizedYield)}{' '}
                  {t('estate.projection.annualized')})
                </span>
              </div>
            </div>
            <div className="flex justify-between items-center mt-1">
              <span className="text-xs text-muted-foreground">
                {t('estate.projection.totalYieldDescription')}
              </span>
              <span className="text-xs text-muted-foreground">
