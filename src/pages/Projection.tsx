...
export default function Projection() {
  ...
  const [editingSettings, setEditingSettings] = useState<
    Record<string, { rate: string; contribution: string }>
  >({});
    ...
    <div className="p-6 md:p-8 lg:p-10 max-w-7xl mx-auto space-y-8">
          ...
          <div className="flex gap-1 p-1 bg-card border rounded-lg">
            {viewTypes.map((vt) => (
              <Button
                ...
                size="sm"
                className={`h-8 px-4 ${
                  viewType === vt ? 'bg-sidebar-accent text-sidebar-accent-foreground' : ''
                }`}
            ...
            <CardContent className="pt-0 pb-4 px-4">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {assetCategories.map((cat) => (
                  ...
                  >
                    <span className="text-sm font-medium">{t(`projection.${cat.labelKey}`)}</span>
                    <div className="text-right text-sm">
                      ...
                      {cat.hasContribution && (
                        <div className="text-muted-foreground text-xs">
                          +
                          {formatCurrency(settingsMap[cat.key]?.contribution ?? 0, {
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <SummaryCard
          ...
          subtitle={
            currentNetWorth > 0
              ? `+${((projectedNetWorth / currentNetWorth - 1) * 100).toFixed(0)}%`
              : undefined
          }
        />
        <SummaryCard
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData} margin={{ top: 10, right: 10, left: 10, bottom: 20 }}>
                ...
                <Area
                  ...
                  fill={colors.savings}
                  fillOpacity={0.6}
                  name={t('projection.categories.savings')}
                />
                <Area
                  type="monotone"
                  dataKey="investments"
                  ...
                  fillOpacity={0.6}
                  name={t('projection.categories.investments')}
                />
                <Area
                  type="monotone"
                  dataKey="crypto"
                  ...
                  fill={colors.crypto}
                  fillOpacity={0.6}
                  name={t('projection.categories.crypto')}
                />
                <Area
                  type="monotone"
                  dataKey="bonds"
                  ...
                  fill={colors.bonds}
                  fillOpacity={0.6}
                  name={t('projection.categories.bonds')}
                />
                <Area
                  type="monotone"
                  dataKey="realEstate"
                  ...
                  fillOpacity={0.6}
                  name={t('projection.categories.realEstate')}
                />
                <Area
                  type="monotone"
                  dataKey="otherAssets"
                  stackId="1"
                  stroke={colors.otherAssets}
                  fill={colors.otherAssets}
                  fillOpacity={0.6}
                  name={t('projection.categories.otherAssets')}
                />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        ...
        <DialogContent className="max-w-xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid gap-6 py-4">
            ...
            <div className="space-y-4">
              <h3 className="text-sm font-semibold">{t('projection.settings.growthRates')}</h3>
              <div className="grid gap-4">
                {assetCategories.map((cat) => (
                  <div key={cat.key} className="grid grid-cols-2 gap-4 items-center">
                    <Label className="text-sm font-medium">{t(`projection.${cat.labelKey}`)}</Label>
                    <div className="flex items-center gap-2">
                      <Input
                        type="number"
            ...
            <div className="space-y-4">
              <h3 className="text-sm font-semibold">{t('projection.settings.contributions')}</h3>
              <div className="grid gap-4">
                {assetCategories
                  .filter((cat) => cat.hasContribution)
                  .map((cat) => (
                    <div key={cat.key} className="grid grid-cols-2 gap-4 items-center">
