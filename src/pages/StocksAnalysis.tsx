...
export default function StocksAnalysis() {
  ...
  const updateTagMutation = useMutation({
    mutationFn: ({
      ...
      id: string;
      data: { name: string; color?: string | null; groupId?: string | null };
  ...
  const filteredMetrics =
    selectedTagIds.length > 0 ? metrics.filter((m) => selectedTagIds.includes(m.tag.id)) : metrics;
  ...
  return (
    <div className="p-6 md:p-8 lg:p-10 max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex flex-wrap justify-between items-center gap-4">
        ...
        <Button variant="outline" size="sm" onClick={() => setIsManagementOpen((prev) => !prev)}>
                  ...
                  <div key={group.id} className="border rounded-lg p-3 space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2 pl-6">
                      ...
                      {tags.filter((t) => t.groupId === group.id).length === 0 && (
                        <span className="text-xs text-muted-foreground">
                          {t('stocksAnalysis.noTagsInGroup')}
                        </span>
                      )}
                    </div>
                  </div>
          ...
          {groups.length > 0 && (
            <div className="flex items-center gap-3">
              <span className="text-sm text-muted-foreground">
                {t('stocksAnalysis.filterByGroup')}:
              </span>
      ...
      {filteredMetrics.length > 0 &&
        (() => {
          ...
          const totalDividend = filteredMetrics.reduce(
            (sum, m) => sum + m.estimatedYearlyDividend,
            0
          );
          ...
          const chartData = [
            ...
            },
            {
              ...
              rawValues: Object.fromEntries(
                filteredMetrics.map((m) => [m.tag.id, Math.abs(m.gainLoss)])
              ),
            },
            {
              ...
              rawValues: Object.fromEntries(
                filteredMetrics.map((m) => [m.tag.id, m.estimatedYearlyDividend])
              ),
            },
          ];
            ...
            {filteredMetrics.map((metric) => (
              <Card key={metric.tag.id} className="overflow-hidden card-hover">
                ...
                <CardHeader className="pb-2">
                  ...
                  <CardDescription>
                    {metric.holdingsCount} {t('stocksAnalysis.holdings')} •{' '}
                    {formatPercent(metric.portfolioPercent)} {t('stocksAnalysis.ofPortfolio')}
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-3">
                  <div className="flex justify-between">
                    <span className="text-sm text-muted-foreground">
                      {t('stocksAnalysis.totalValue')}
                    </span>
                    <span className="font-medium">{formatCurrency(metric.totalValue)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm text-muted-foreground">
                      {t('stocksAnalysis.gainLoss')}
                    </span>
                    <span
                      className={`font-medium ${metric.gainLoss >= 0 ? 'text-green-600' : 'text-red-600'}`}
                    >
                  ...
                  <div className="flex justify-between">
                    <span className="text-sm text-muted-foreground">
                      {t('stocksAnalysis.yearlyDividend')}
                    </span>
                    <span className="font-medium">
                      {formatCurrency(metric.estimatedYearlyDividend)}
                    </span>
                  </div>
                </CardContent>
              </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {t(
              'stocksAnalysis.twrDescription',
              'Time-weighted return removes distortion caused by buy/sell transactions'
            )}
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <div className="flex flex-wrap items-center gap-2 flex-1">
              ...
              >
                <span
                  className="w-2 h-2 rounded-full mr-1.5"
                  style={{
                    backgroundColor: twrShowUntagged ? '#fff' : 'hsl(var(--muted-foreground))',
                  }}
                />
      ...
      <Dialog open={isCreateTagDialogOpen} onOpenChange={setIsCreateTagDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>{t('stocksAnalysis.createTagDescription')}</DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={isCreateGroupDialogOpen} onOpenChange={setIsCreateGroupDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>{t('stocksAnalysis.createGroupDescription')}</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
