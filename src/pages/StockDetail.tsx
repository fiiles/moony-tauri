...
import TickerValueTrendChart, {
  type TransactionMarker,
} from '@/components/common/TickerValueTrendChart';
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="animate-pulse text-muted-foreground">{tc('status.loading')}</div>
      </div>
    );
  }
  ...
  if (!investment) {
    return (
      <div className="p-6">
        ...
        <div className="mt-8 text-center text-muted-foreground">{t('detail.notFound')}</div>
      </div>
    );
  }
    ...
    <div className="p-4 sm:p-6 max-w-6xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <span className="flex items-center gap-1 flex-wrap">
                ...
                {avgCostCurrency !== currencyCode && (
                  <span className="text-xs">
                    (≈{' '}
                    {formatCurrencyRaw(avgPrice, {
                      minimumFractionDigits: avgPrice >= 1000 ? 0 : 2,
                      maximumFractionDigits: avgPrice >= 1000 ? 0 : 2,
                    })}
                    )
                  </span>
                )}
        ...
        <Card className="card-hover">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">
              {t('detail.unrealizedPnL')}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div
              className={`text-2xl font-bold flex items-center gap-1 ${unrealizedPnL >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
            >
              {unrealizedPnL >= 0 ? (
            ...
            <div
              className={`text-sm ${unrealizedPnL >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
            >
              {unrealizedPnL >= 0 ? '+' : ''}
        ...
        <Card className="card-hover">
          ...
          <CardContent>
            ...
            <div className="text-sm text-muted-foreground">
              {yearlyDividend > 0
                ? `${formatCurrencyRaw(yearlyDividend)} / ${t('detail.year')}`
                : t('detail.noDividend')}
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <div className="text-lg font-medium">
                {investment.fetchedAt
                  ? formatDate(new Date(Number(investment.fetchedAt) * 1000))
                  : '-'}
              </div>
            </div>
          </div>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        {hasMixedCurrencies && (
                          <TableHead className="text-right">
                            {t('detail.transactions.originalPrice')}
                          </TableHead>
                        )}
                        <TableHead className="text-right">
                    ...
                    <TableBody>
                      {transactions.map((tx) => {
                        ...
                        const formattedOriginalPrice =
                          originalCurrencyDef.position === 'before'
                        ...
                        const displayQty = Number.isInteger(txQty)
                          ? txQty.toFixed(0)
                          : txQty.toFixed(4);
                        return (
                          <TableRow key={tx.id}>
                            ...
                            <TableCell>
                              <span
                                className={`px-2 py-1 rounded text-xs font-medium ${tx.type === 'buy' ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' : 'bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300'}`}
                              >
