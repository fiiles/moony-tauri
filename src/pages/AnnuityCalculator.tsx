  ...
  const chartData = useMemo(() => {
    ...
    if (chartView === 'yearly' || periodicity === 'annually') {
      const yearlyData: Record<
        number,
        { label: string; principal: number; interest: number; balance: number }
      > = {};
        ...
        <CardContent className="pt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="space-y-2">
              ...
              <Select
                value={periodicity}
                onValueChange={(v) => setPeriodicity(v as PaymentPeriodicity)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="monthly">
                    {t('annuity.form.periodicityOptions.monthly')}
                  </SelectItem>
                  <SelectItem value="quarterly">
                    {t('annuity.form.periodicityOptions.quarterly')}
                  </SelectItem>
                  <SelectItem value="semiAnnually">
                    {t('annuity.form.periodicityOptions.semiAnnually')}
                  </SelectItem>
                  <SelectItem value="annually">
                    {t('annuity.form.periodicityOptions.annually')}
                  </SelectItem>
                </SelectContent>
              </Select>
            </div>
          ...
          <Card className="bg-primary/5 border-primary/20">
            <CardContent className="pt-6">
              ...
              <p className="text-3xl font-bold text-primary">
                {formatCurrencyRaw(result.periodicPayment)}
              </p>
          ...
          <Card>
            <CardContent className="pt-6">
              ...
              <p className="text-2xl font-semibold text-orange-600 dark:text-orange-400">
                {formatCurrencyRaw(result.totalInterest)}
              </p>
            </CardContent>
          </Card>
        </div>
              ...
              <AreaChart data={chartData} margin={{ top: 10, right: 10, left: 10, bottom: 20 }}>
                ...
                <Tooltip
                  formatter={(value, name) => [
                    ...
                    name === 'principal'
                      ? t('annuity.chart.principal')
                      : t('annuity.chart.interest'),
                  ]}
                  contentStyle={{
                  ...
                  <TableHeader className="sticky top-0 bg-background">
                    <TableRow>
                      ...
                      <TableHead className="text-right">
                        {t('annuity.table.principalPayment')}
                      </TableHead>
                      <TableHead className="text-right">
                        {t('annuity.table.interestPayment')}
                      </TableHead>
                      <TableHead className="text-right">
                        {t('annuity.table.remainingBalance')}
                      </TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {result.schedule.map((row: AmortizationRow) => (
                      <TableRow key={row.periodNumber}>
                        ...
                        <TableCell className="text-right">
                          {formatCurrencyRaw(row.payment)}
                        </TableCell>
                        <TableCell className="text-right text-primary">
                          {formatCurrencyRaw(row.principalPayment)}
                        </TableCell>
                        <TableCell className="text-right text-orange-600 dark:text-orange-400">
                          {formatCurrencyRaw(row.interestPayment)}
                        </TableCell>
                        <TableCell className="text-right">
                          {formatCurrencyRaw(row.remainingBalance)}
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
